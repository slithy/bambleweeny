...
class Character(BaseCharacter):
    ...
    def setAlignment(self, alignment: str):
        ...
        if len(alignment) != 2:
            raise InvalidArgument(
                f"Alignment must be specified as two letters: [L]awful/[N]eutral/[C]haotic + [G]ood/[N]eutral/[E]vil"
            )
        law_axis = alignment[0].upper()
        evil_axis = alignment[1].upper()
        if law_axis != "L" and law_axis != "N" and law_axis != "C":
            raise InvalidArgument(
                "First letter of alignment must be [L]awful/[N]eutral/[C]haotic"
            )
        if evil_axis != "G" and evil_axis != "N" and evil_axis != "E":
            raise InvalidArgument(
                "Second letter of alignment must be [G]ood/[N]eutral/[E]vil"
            )
    ...
    def getClassPrime(self):
        if self.xclass == "Fighter" or self.xclass == "Ranger":
            ...
            self.xclass == "Knight" or self.xclass == "Paladin" or self.xclass == "Bard"
        ):
            return "cha"
    ...
    def setPrimes(self, first_prime: str, second_prime: str, third_prime: str):
        ...
        if self.race == "Human":
            if second_prime == "None":
                raise InvalidArgument(
                    f"Human characters must specify two primes (in addition to class prime)."
                )
            ...
            if third_prime != "None":
                raise InvalidArgument(
                    f"Non-human characters must specify one prime (in addition to class prime)."
                )

        if (
            prime_1 != self.getClassPrime()
            and prime_2 != self.getClassPrime()
            and prime_3 != self.getClassPrime()
        ):
            raise InvalidArgument(
                f"One of your primes must be your class prime ({self.getClassPrime()})"
            )

        if prime_1 == prime_2 or prime_1 == prime_3 or prime_2 == prime_3:
            raise InvalidArgument(
                f"Can't set primes for a {self.race} {self.xclass} to {prime_1} {prime_2} {prime_3}"
            )
    ...
    def getAC(self):
        ...
        if self.race == "Half-Orc" and (
            self.xclass in {"Monk", "Wizard", "Illusionist"}
        ):
            ac += 1
        return ac
    ...
    def getBtH(self):
        if self.xclass == "Fighter":
            ...
            self.xclass == "Ranger"
            or self.xclass == "Barbarian"
            or self.xclass == "Monk"
            or self.xclass == "Knight"
            or self.xclass == "Paladin"
            or self.xclass == "Bard"
        ):
            return self.level - 1
        elif self.xclass == "Cleric" or self.xclass == "Druid":
    ...
    def showCharacter(self, message: str = ""):
        return (
            f"{self.name}, {self.race} {self.xclass}, {self.getAlignment()}, Level: {self.level} {message}, "
            f"God: {self.god}\n"
