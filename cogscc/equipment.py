...
class Equipment:
    def __init__(
        ...
        if ev < 0.002:
            raise OutOfRange(
                "Items with no appreciable EV: treat the EV as 1 per 10 items carried (PHB p.46)"
            )
        if count < 1:
    ...
    def isPushable(self, e):
        return (
            ...
            if type(self) == type(e)
            and self.ev == e.ev
            and self.value == e.value
            and self.gm_note == e.gm_note
            else False
        )
    ...
    def show(self, options: list = []):
        ...
        gm_note = (
            " :small_blue_diamond:" if "gm_note" in options and self.gm_note else ""
        )
    ...
    def show(self, options: list = []):
        if not self.is_wielding or "detail" in options:
            ...
            dmg = (
                f"{self.damage} dmg"
                if self.bth == 0
                else f"BtH {self.bth:+}, {self.damage} dmg"
            )
...
class Ammo(Equipment):
    def __init__(
        self, description: str, count: int, dmg: str, bth: int, ev: float, value: int
    ):
...
class Container(Equipment):
    ...
    def put(self, item):
        if item.isContainer():
            raise NestedContainer(
                f"You try to put {item.show()} inside {self.show()}. This tears a hole in the fabric of space-time. You are sucked in and die. :skull:"
            )
    ...
    def getEV(self):
        return int(sum([item.getEV() for item in self.contents]) / 2) + (
            self.ev * self.count
        )
    ...
    def show(self, options: list = []):
        if "detail" in options:
            return super().show(options)
        ev = (
            f"  Capacity {self.capacity*self.count}, EV {int(self.getEV() + 0.5)}"
            if "ev" in options
            else ""
        )
        value = f" ({self.value * self.count} gp)" if self.value > 0 else ""
...
class Coin:
    ...
    def __init__(self):
        self.coin = {"pp": 0, "gp": 0, "sp": 0, "ep": 0, "cp": 0}
    ...
    def drop(self, amount: int, denomination: str):
        ...
        if self.coin[denomination] == 0:
            raise CreditLimitExceeded(
                f"You look in your purse but find no {denomination}"
            )
        elif self.coin[denomination] < amount:
            raise CreditLimitExceeded(
                f"Your purse contains only {self.coin[denomination]} {denomination}"
            )
        else:
            self.coin[denomination] -= amount
...
class EquipmentList:
    ...
    def find(self, description: str, exactMatch: bool = False, inlist: list = []):
        if not inlist:
            ...
            # exact match
        found_items = [
            idx for idx, i in enumerate(inlist) if i.lower() == description.lower()
        ]
        # # If we didn't find one, try partial matches at the start of the string
        if len(found_items) == 0 and not exactMatch:
            found_items = [
                idx
                for idx, i in enumerate(inlist)
                if i.lower().startswith(description.lower())
            ]
            # If we still didn't find one, try partial matches anywhere in the string
        if len(found_items) == 0 and not exactMatch:
            found_items = [
                idx for idx, i in enumerate(inlist) if description.lower() in i.lower()
            ]
        ...
        if len(found_items) == 0:
            ...
            desc_list = "\n".join([inlist[i].description for i in found_items])
            raise AmbiguousMatch(
                f"{description} matches more than one item, please be more specific. The matching "
                f"items are: \n{desc_list}"
            )
    ...
    def swapWeapons(self):
        ...
        idx0 = wi[0][1]
        idx1 = wi[1][1]
        self.equipment[idx1], self.equipment[idx0] = (
            self.equipment[idx0],
            self.equipment[idx1],
        )
    ...
    def freeHands(self, hands: int, name: str):
        ...
        for item_no in range(len(self.equipment)):
            if (
                self.equipment[item_no].isWearing()
                or self.equipment[item_no].isWielding()
            ):
        ...
        for item_no in range(len(self.equipment)):
            if (
                self.equipment[item_no].isWearing()
                and self.equipment[item_no].hands > 0
            ):
    ...
    def add(self, description: str, d: dict):
        ...
        if itemno < 0:
            ...
            raise UniqueItem(
                f"You already have {self.equipment[itemno].show()} but the type of item is different."
            )
    ...
    def isPushable(self, description: str, e):
        ...
        return itemno_dest < 0 or self.equipment[itemno_src].isPushable(
            self.equipment[itemno_dest]
        )
    ...
    def addWeapon(self, description: str, d: dict):
        ...
        if itemno < 0:
            ...
            raise UniqueItem(
                f"Weapons are unique and you already have {self.equipment[itemno].show()}."
            )
    ...
    def wield(self, name: str, description: str):
        ...
        if itemno < 0:
            ...
            raise ItemNotWieldable(
                f"{self.equipment[itemno].show()} is not something you can wield."
            )
    ...
    def addWearable(self, description: str, d: dict):
        ...
        if itemno < 0:
            ...
            raise UniqueItem(
                f"Wearable items are unique and you already have {self.equipment[itemno].show()}."
            )
    ...
    def wear(self, name: str, description: str, location: str = ""):
        ...
        if itemno < 0:
            ...
            raise ItemNotWearable(
                f"{self.equipment[itemno].show()} is not something you can wear."
            )
        else:
            reply = self.freeHands(self.equipment[itemno].hands, name)
    ...
    def takeOff(self, description: str):
        ...
        if itemno < 0:
            ...
            raise NotWearingItem(
                f"You are not bearing {self.equipment[itemno].show()}."
            )
        return reply
        ...
        if itemno < 0:
            if not d.get("ev", ""):
                raise MissingArgument(
                    f"You need to specify the Encumbrance Value (EV) for {description}."
                )
            if not d.get("capacity", ""):
                raise MissingArgument(
                    f"You need to specify the capacity for {description}."
                )
            newitem = Container(
                description,
                d.get("count", 1),
            ...
            raise ItemNotMutable(
                f"Capacity:{d.get('capacity')} is different from the capacity of {self.equipment[itemno].show()}"
            )
        elif isAttrDifferent(d, "ev", self.equipment[itemno].ev):
            raise ItemNotMutable(
                f"EV:{d.get('ev')} is different from the EV of {self.equipment[itemno].show(['ev'])}"
            )
        elif isAttrDifferent(d, "value", self.equipment[itemno].value):
            raise ItemNotMutable(
                f"Value:{d.get('value')} is different from the value of {self.equipment[itemno].show()}"
            )
        else:
    ...
    def put(self, description: str, container: str):
        ...
        return (
            f"puts {self.equipment[itemno].show()} into {self.equipment[contno].show()}"
        )
    ...
    def pickUp(self, description: str):
        ...
        if itemno < 0:
            ...
            raise ItemNotFound(
                f"{self.equipment[itemno].show()} is not temporarily dropped."
            )
        else:
            self.equipment[itemno].description = self.equipment[
                itemno
            ].description.replace(" [dropped]", "")
    ...
    def getInventory(self, section: str = "", options: list = []):
        ...
        section_list.extend(
            sorted([item.description for item in self.equipment if item.isContainer()])
        )
        ...
        if section and section.lower() != "all":
            ...
            if sectionno < 0:
                raise InventorySectionNotFound(
                    f'I don\'t know what you mean by "{section}" (not a container).'
                )
        ...
        for item in self.equipment:
            if item.isWearing() and "Wearing" in section_dict:
                ...
                not item.isWearing()
                and not item.isWielding()
                and not item.isContainer()
                and not item.isInContainer()
        ...
        for section_key, item_list in section_dict.items():
            if not item_list:
                if len(section_dict) > 1 and section_key in [
                    "Wearing",
                    "Wielding",
                    "Carrying",
                    "Treasure",
        ...
        if not self.coin.empty() and (
            not section or section.lower() == "all" or "Treasure" in section_dict
        ):
