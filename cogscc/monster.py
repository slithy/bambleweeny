...
class Monster(BaseCharacter):
    def __init__(self, name: str, d: dict):
        ...
        self.hp = [
            HP(hp) if type(hp) is int else HP.__from_dict__(hp) for hp in hp_list
        ]
        # Saves
    ...
    def isMatchName(self, name: str):
        ...
        return personal_name.lower().startswith(
            name.lower()
        ) or self.name.lower().startswith(name.lower())

    def rollHp(self, already_rolled: int = 0):
        result = [
            roll(f"{self.hd}", inline=True) for _ in range(self.count - already_rolled)
        ]
        for hp in result:
            print(f"{hp.skeleton}")
    ...
    def showSummary(self, msg: str = ""):
        ...
        if self.count == 1:
            ...
            desc = self.name + "s"
        return (
            f"{msg}{personal_name} the {self.name}"
            if personal_name
            else f"{number}{desc}"
        )
